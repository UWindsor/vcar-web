...
class AuthDB:
    ...
    def firstConstructor(self, email: str, password: str, token: str, number_of_vehicles: str): 
        self.email = email
        self.password = password
        self.token = token
        self.number_of_vehicles = number_of_vehicles
    
    def secondaryConstructor(self, email: str, token: str):
        self.email = email
        self.token = token
    ...
    def getVehicleNumber(self):
        cursor = self.connection.cursor()
        cursor.execute('''SELECT number_of_vehicles FROM auth_token WHERE email=? AND token=?''', (self.email, self.token,))
